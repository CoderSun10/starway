...
function parseYmd(str) {
  if (typeof str !== 'string' || !YMD.test(str)) return null;
  const [, ys, ms, ds] = str.match(YMD);
  const y = +ys;
  const m = +ms;
  const d = +ds;
  ...
  if (str < DAY_MIN || str > DAY_MAX) return null;
  return str;
}
...
/** year, month 为上海日历 1-based。周一为首；去掉整行都是下月的尾巴。 */
function visibleMonthRange(year, month) {
  const first = new Date(Date.UTC(year, month - 1, 1));
  const last = new Date(Date.UTC(year, month, 0));
  const mondayOffset = (first.getUTCDay() + 6) % 7;
  const from = new Date(first);
  from.setUTCDate(1 - mondayOffset);
  const prefix = mondayOffset;
  const daysInMonth = last.getUTCDate();
  const weeks = Math.ceil((prefix + daysInMonth) / 7);
